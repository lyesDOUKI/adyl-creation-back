...
class GetOrderServiceIntegrationTest {
    ...
    void findById_returns_success_with_mapped_order_and_lines_when_order_exists() {
        ...
        insertOrder(orderId, customerId, "Livrer avant vendredi", BigDecimal.valueOf(35.82));
        ...
        assertThat(response.customerMessage()).isEqualTo("Livrer avant vendredi");
        assertThat(response.total()).isEqualByComparingTo(BigDecimal.valueOf(35.82));
        assertThat(response.status()).isEqualTo("PENDING");
        assertThat(response.lineCount()).isEqualTo(1);
        assertThat(response.totalQuantity()).isEqualByComparingTo(BigDecimal.valueOf(2));
        ...
        assertThat(line.productCategory()).isEqualTo(ProductCategoryResponse.CLOTHING);
        assertThat(line.quantity()).isEqualByComparingTo(BigDecimal.valueOf(2));
        assertThat(line.unitPrice()).isEqualByComparingTo(BigDecimal.valueOf(19.90));
        assertThat(line.chosenColor()).isEqualTo("Rouge");
        assertThat(line.discountRate()).isEqualByComparingTo(BigDecimal.valueOf(0.10));
        assertThat(line.totalAmount()).isEqualByComparingTo(BigDecimal.valueOf(35.82));
    }

    @Test
    void findById_aggregates_multiple_lines_for_the_same_order() {
        UUID productA = UUID.randomUUID();
        insertProduct(productA, "T-shirt", ProductCategory.CLOTHING, BigDecimal.valueOf(19.90));

        UUID productB = UUID.randomUUID();
        insertProduct(productB, "Mug", ProductCategory.ACCESSORIES, BigDecimal.valueOf(9.90));

        UUID customerId = UUID.randomUUID();
        insertCustomer(customerId);

        UUID orderId = UUID.randomUUID();
        insertOrder(orderId, customerId, null, BigDecimal.valueOf(29.80));

        insertOrderDetail(orderId, productA, BigDecimal.ONE, BigDecimal.valueOf(19.90),
                BigDecimal.valueOf(19.90), "Bleu", BigDecimal.ZERO);
        ...
        assertThat(response.totalQuantity()).isEqualByComparingTo(BigDecimal.valueOf(2));
        assertThat(response.lines()).extracting(OrderLineResponse::productName)
    ...
    void findAll_maps_every_order_and_preserves_paging_metadata() {
        ...
        insertOrder(orderA, customerId, "A", BigDecimal.valueOf(19.90));
        insertOrderDetail(orderA, productId, BigDecimal.ONE, BigDecimal.valueOf(19.90),
                BigDecimal.valueOf(19.90), "Rouge", BigDecimal.ZERO);

        UUID orderB = UUID.randomUUID();
        insertOrder(orderB, customerId, "B", BigDecimal.valueOf(39.80));
    ...
    private void insertProduct(UUID productId, String name, ProductCategory category, BigDecimal price) {
        dsl.insertInto(PRODUCTS)
                .columns(PRODUCTS.ID, PRODUCTS.NAME, PRODUCTS.UNIT_PRICE, PRODUCTS.STATUS, PRODUCTS.CATEGORY)
                .values(productId, name, price, String.valueOf(ProductStatus.AVAILABLE), String.valueOf(category))
                .execute();
    }
    ...
    private void insertCustomer(UUID identitySubject) {
        dsl.insertInto(CUSTOMERS, CUSTOMERS.ID, CUSTOMERS.IDENTITY_SUBJECT,
                ...
                .values(UUID.randomUUID(), identitySubject,  "jean.dupont+" + identitySubject + "@test.com", "0600000000")
                .execute();
    }

    private void insertOrder(UUID orderId, UUID customerId, String customerMessage, BigDecimal total) {
        dsl.insertInto(ORDERS, ORDERS.ID, ORDERS.CUSTOMER_ID, ORDERS.CUSTOMER_MESSAGE,
                        ORDERS.TOTAL, ORDERS.STATUS_TYPE, ORDERS.STATUS_DATA)
                .values(orderId, customerId, customerMessage, total, "PENDING",
                        JSON.json(converter.convertToDatabaseColumn(OrderStatus.PENDING)))
                .execute();
    }

    private void insertOrderDetail(UUID orderId, UUID productId, BigDecimal quantity,
                                   BigDecimal unitPrice, BigDecimal totalAmount,
                                   String chosenColor, BigDecimal discountRate) {
        dsl.insertInto(ORDER_DETAILS, ORDER_DETAILS.ID, ORDER_DETAILS.ORDER_ID, ORDER_DETAILS.PRODUCT_ID,
                        ORDER_DETAILS.QUANTITY, ORDER_DETAILS.UNIT_PRICE, ORDER_DETAILS.TOTAL_AMOUNT,
                        ORDER_DETAILS.CHOSEN_COLOR, ORDER_DETAILS.DISCOUNT_RATE)
                .values(UUID.randomUUID(), orderId, productId, quantity, unitPrice, totalAmount,
                        chosenColor, discountRate)
                .execute();
    }
}